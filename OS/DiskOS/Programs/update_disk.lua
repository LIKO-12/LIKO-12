...
if not source then color(9) print("Must provide path to the file to update") return end
...
if filetype ~= "OSData" then
  color(9) print("Can't update '"..filetype.."' files !") return
end

local osname = nextarg()
if not osname then color(9) print("Invalid Data !") return end
if osname ~= "DiskOS" then color(9) print("Can't update files from '"..osname.."' OS !") return end

local datatype = nextarg()
if not datatype then color(9) print("Invalid Data !") return end
if datatype ~= "DiskGame" then color(9) print("Can't update '"..datatype.."' from '"..osname.."' OS !") return end

local dataver = nextarg()
if not dataver then color(9) print("Invalid Data !") return end
dataver = tonumber(string.match(dataver,"V(%d+)"))
if not dataver then color(9) print("Invalid Data !") return end
if dataver == _DiskVer then color(9) print("Disk is already up to date !") return end
if dataver > 1 then color(9) print("Can't update disks newer than V1, provided: V"..dataver) return end
if dataver < 1 then color(9) print("Can't update disks older than V1, provided: V"..dataver) return end
...
color(12) print("Updated to Disk V".._DiskVer.." Successfully")
